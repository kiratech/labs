...
This will take long to complete (5 to 15 minutes) and will generate four files
under the `generated` folder:

```console
(venv) $ ls -1rt generated/
discarded_merlinite-7b-lab-Q4_K_M_2024-06-07T09_34_21.log
generated_merlinite-7b-lab-Q4_K_M_2024-06-07T09_34_21.json
train_merlinite-7b-lab-Q4_K_M_2024-06-07T09_34_21.jsonl
test_merlinite-7b-lab-Q4_K_M_2024-06-07T09_34_21.jsonl
```
...
4. Download the LoRA `ggml-adapter-model.bin` file generated by the run that
   was put inside the `` colab folder.
...
```console
(venv) $ llama.cpp/export-lora \
         -m $HOME/Labs/ilab/models/ggml-model-f16.gguf \
         -l $HOME/Labs/ilab/ggml-adapter-model.bin \
         -o $HOME/Labs/ilab/models/ggml-model-f16-custom.gguf
```

And once the generation is complete the `models/ggml-model-f16-custom.gguf` will
be usable as follows:

```console
(venv) $ ilab --config=$HOME/Labs/instructlab/config.yaml \
         serve \
...
Results are now better, and a new workflow could be run to improve again the
model.
